...
class ServicoDataTable extends DataTable
{
     ...
     * 
     * @param QueryBuilder<Servico> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('Editar', function($query){
                $editar = '<a href="'.route('servicos.edit', $query->id).'" class="btn btn-primary"><i class="fas fa-edit"></i></a>';
                $deletar = '<a href="'.route('servicos.destroy', $query->id).'" class="btn btn-primary delete-item ml-2 mr-2" ><i class="fa fa-trash-alt"></i></a>';
